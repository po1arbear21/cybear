...
contains
  ...
  contains
    ...
    subroutine case_1b(j, djdeta, djddpot)
      ...
      integer :: ncalls
      real    :: I(-5:0), dI(2,-5:0), jc(1:5), djc(2,1:5), tmp, dtmp(2)
      ...
      jc(1)    = I(0) / I(-1)
      djc(:,1) = (dI(:,0) - dI(:,-1) * jc(1)) / I(-1)

      tmp      = I(-1)**2 - I(-2)*I(0)
      dtmp     = 2 * I(-1) * dI(:,-1) - dI(:,-2)*I(0) - I(-2)*dI(:,0)
      jc(2)    = tmp / I(-1)**3
      djc(:,2) = (dtmp - 3 * tmp * dI(:,-1) / I(-1)) / I(-1)**3

      tmp      = 2*I(-2)**2*I(0) - I(-1)**2*I(-2) - I(-1)*I(-3)*I(0)
      dtmp     = 4*I(-2)*dI(:,-2)*I(0) + 2*I(-2)**2*dI(:,0) - 2*I(-1)*dI(:,-1)*I(-2) - I(-1)**2*dI(:,-2) &
        &      - dI(:,-1)*I(-3)*I(0) - I(-1)*dI(:,-3)*I(0) - I(-1)*I(-3)*dI(:,0)
      jc(3)    = tmp / I(-1)**5
      djc(:,3) = (dtmp - 5 * tmp * dI(:,-1) / I(-1)) / I(-1)**5

      tmp      = 2*I(-1)**2*I(-2)**2 - I(-1)**3*I(-3) - 5*I(-2)**3*I(0) - I(-1)**2*I(-4)*I(0) + 5*I(-1)*I(-2)*I(-3)*I(0)
      dtmp     = 4*I(-1)*dI(:,-1)*I(-2)**2 + 4*I(-1)**2*I(-2)*dI(:,-2) - 3*I(-1)**2*dI(:,-1)*I(-3) - I(-1)**3*dI(:,-3) &
        &      - 15*I(-2)**2*dI(:,-2)*I(0) - 5*I(-2)**3*dI(:,0) - 2*I(-1)*dI(:,-1)*I(-4)*I(0) - I(-1)**2*dI(:,-4)*I(0) &
        &      - I(-1)**2*I(-4)*dI(:,0) + 5*dI(:,-1)*I(-2)*I(-3)*I(0) + 5*I(-1)*dI(:,-2)*I(-3)*I(0) &
        &      + 5*I(-1)*I(-2)*dI(:,-3)*I(0) + 5*I(-1)*I(-2)*I(-3)*dI(:,0)
      jc(4)    = tmp / I(-1)**7
      djc(:,4) = (dtmp - 7 * tmp * dI(:,-1) / I(-1)) / I(-1)**7


      tmp      = 14*I(0)*I(-2)**4 - I(-1)**4*I(-4) - 5*I(-1)**2*I(-2)**3 - I(0)*I(-1)**3*I(-5) &
        &      + 5*I(-1)**3*I(-2)*I(-3) + 3*I(0)*I(-1)**2*I(-3)**2 - 21*I(0)*I(-1)*I(-2)**2*I(-3) &
        &      + 6*I(0)*I(-1)**2*I(-2)*I(-4)
      dtmp     = 14*dI(:,0)*I(-2)**4 + 56*I(0)*I(-2)**3*dI(:,-2) - 4*I(-1)**3*dI(:,-1)*I(-4) - I(-1)**4*dI(:,-4) &
        &      - 10*I(-1)*dI(:,-1)*I(-2)**3 - 15*I(-1)**2*I(-2)**2*dI(:,-2) - dI(:,0)*I(-1)**3*I(-5) &
        &      - 3*I(0)*I(-1)**2*dI(:,-1)*I(-5) - I(0)*I(-1)**3*dI(:,-5) + 15*I(-1)**2*dI(:,-1)*I(-2)*I(-3) &
        &      + 5*I(-1)**3*dI(:,-2)*I(-3) + 5*I(-1)**3*I(-2)*dI(:,-3) + 3*dI(:,0)*I(-1)**2*I(-3)**2 &
        &      + 6*I(0)*I(-1)*dI(:,-1)*I(-3)**2 + 6*I(0)*I(-1)**2*I(-3)*dI(:,-3) - 21*dI(:,0)*I(-1)*I(-2)**2*I(-3) &
        &      - 21*I(0)*dI(:,-1)*I(-2)**2*I(-3) - 42*I(0)*I(-1)*I(-2)*dI(:,-2)*I(-3) - 21*I(0)*I(-1)*I(-2)**2*dI(:,-3) &
        &      + 6*dI(:,0)*I(-1)**2*I(-2)*I(-4) + 12*I(0)*I(-1)*dI(:,-1)*I(-2)*I(-4) + 6*I(0)*I(-1)**2*dI(:,-2)*I(-4) &
        &      + 6*I(0)*I(-1)**2*I(-2)*dI(:,-4)
      jc(5)    = tmp / I(-1)**9
      djc(:,5) = (dtmp - 9 * tmp * dI(:,-1) / I(-1)) / I(-1)**9

      j       = 0
      djdeta  = 0
      djddpot = 0
      do k = 1, 5
        j       = j + jc(k) * (dpot_ - I(0))**k
