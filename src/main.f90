...
block
  ...
  !   print "(I6,ES24.16)", ii, denorm(dev%par%g1D(1)%x(ii), "nm")
  ! end do
  ! do ii = 1, dev%par%g1D(1)%n
  !   print "(2ES24.16)", denorm(dev%par%g1D(1)%x(ii), "nm"), denorm(dev%pot%x1(ii), "V")
  ! end do
  ! do ii = 1, dev%par%g1D(2)%n
  !   print "(2ES24.16)", denorm(dev%par%g1D(2)%x(ii), "nm"), denorm(dev%pot%x2(25,ii), "V")
  ! end do
  ! do ii = 1, dev%par%g1D(1)%n
  !   print "(2ES24.16)", denorm(dev%par%g1D(1)%x(ii), "nm"), denorm(dev%pot%x3(ii,25,13), "V")
  ! end do
  ! print*, "z"
  ! do ii = 1, dev%par%g1D(3)%n
  !   print "(I6,ES24.16)", ii, denorm(dev%par%g1D(3)%x(ii), "nm")
  ! end do
  ! print*, "y"
  ! do ii = 1, dev%par%g1D(2)%n
  !   print "(I6, ES24.16)", ii, denorm(dev%par%g1D(2)%x(ii), "nm")
  ! end do
  ! print*, "x"
  ! do ii = 1, dev%par%g1D(1)%n
  !   print "(I6,ES24.16)", ii, denorm(dev%par%g1D(1)%x(ii), "nm")
  ...
  ! do ii = 1, dev%par%g1D(2)%n
  !   print "(2ES24.16)", denorm(dev%par%g1D(2)%x(ii), "nm"), denorm(dev%pot%x2(25,ii), "V")
  ! !   ! print "(2ES24.16)", denorm(dev%par%g1D(1)%x(ii), "nm"), denorm(dev%par%dop(IDX_VERTEX,0,1)%get([ii,25]), "1/cm^3")
  ! end do

  ! open (newunit = funit, file = "trxy", status = "replace", action = "write")
  ...
  ! allocate (idx(dev%par%g%idx_dim), idx1(dev%par%g%idx_dim), idx2(dev%par%g%idx_dim))
  ! allocate (p(dev%par%g%dim,2))
  ...
  !     call dev%par%g%get_edge(idx, idx_dir, p)
  !     write (funit, "(3ES24.16)") denorm(p(1,1), "nm"), denorm(p(2,1), "nm"), denorm(dev%pot%get(idx1), "V")
  ...
  ! write (funit, "(A)") "$ DATA=CURVE3D"
  ! write (funit, *)
  ! do ii = 1, dev%par%gtr%nvert
  !   write (funit, "(A)") "% mt=4"
  !   call dev%par%gtr%get_vertex([ii], xy1)

  !   check double vertices

  !   write (funit, "(3ES24.16)") denorm(xy1(1), "nm"), denorm(xy1(2), "nm"), denorm(dev%par%dop(IDX_EDGE,2,1)%get([ii,9]), "1/cm^3")
  !   write (funit, *)
  ! end do
  ! write (funit, "(A)") "$ END"
