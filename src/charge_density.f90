...
contains
  ...
  subroutine calc_charge_density_init(this, par, dens, ion, rho)
    ...
    do ci = par%smc(par%smc_default)%ci0, par%smc(par%smc_default)%ci1
    ...
    do i = 1, this%par%transport(IDX_VERTEX,0)%n
      ...
      do ci = DOP_DCON, DOP_ACON
        if (this%par%dopvert(ci)%flags%get(idx)) then
          ...
          if (this%par%smc(this%par%smc_default)%incomp_ion .and. (ci >= this%par%smc(this%par%smc_default)%ci0) .and. (ci <= this%par%smc(this%par%smc_default)%ci1)) then
